...
const state: NewsStandState = {
  ...
  headlines: [
    {
      pressName: "연합뉴스",
      headlineText: "김성태와 공모해 대북송금, 안부수 아태협 회장 징역 3년6월",
    },
    {
      pressName: "연합뉴스",
      headlineText: "국가자격시험서 어이없는 사고…채점 안한 609명 답안지 파쇄",
    },
    {
      pressName: "연합뉴스",
...
function onStateChanged(state: NewsStandState) {
  newsStand.render({
    headlineLeft: state.headlines[state.leftHeadlineIndex]!,
    headlineRight: state.headlines[state.rightHeadlineIndex]!,
    systemDate: state.systemDate,
  });
  if (root.firstElementChild !== newsStand.element) {
    root.firstElementChild?.remove();
    root.appendChild(newsStand.element);
  }
}
